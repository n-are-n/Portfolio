...
const Validate = () : void => { const client : Object = { Name : <HTMLInputElement>document.getElementById('Name'), Mail : <HTMLInputElement>document.getElementById('Mail'), Comment : <HTMLInputElement>document.getElementById('Comment'), Date : new Date()}
    for (const key in client) { const URL : string = `https://api.telegram.org/bot5442760054:AAGNKmQ3Rc2VC4o7QiUCo-o4J-UsVbL8p74/sendMessage?chat_id=833384210&text=${key} : ${ key === 'Date' ? client[key] : client[key].value}`
        fetch(URL)}
