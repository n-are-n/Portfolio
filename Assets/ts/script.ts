...
const scrollActive = () : void => { const sections : NodeListOf<HTMLElement> = document.querySelectorAll('section[id]')
    sections.forEach(section  => { const link : HTMLElement = document.querySelector(`.nav__menu a[href*='${section.getAttribute('id')}']`), scrollY :number  = window.scrollY, sectionHeight : number = section.offsetHeight, sectionTop : number = section.offsetTop - 50
        scrollY > sectionTop && scrollY <= sectionTop + sectionHeight ? link.classList.add('active') : link.classList.remove('active')})}
window.addEventListener('scroll', scrollActive)
const Validate = () : void => { const client : Object = { Name : <HTMLInputElement>document.getElementById('Name'), Mail : <HTMLInputElement>document.getElementById('Mail'), Comment : <HTMLInputElement>document.getElementById('Comment'), Date : new Date()}
